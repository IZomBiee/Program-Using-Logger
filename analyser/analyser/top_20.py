...
data = DataLoader(
    input("Write starting date (for ex. 06_03_2023 or nothing) -> "),
...
data_sums = dict(
    sorted(
        {name: round(seconds / 3600, 2) for name, seconds in data_sums.items()}.items(),
...
for bar in bars:
    width = bar.get_width()
    plt.text(
        width + 1, bar.get_y() + bar.get_height() / 2, f"{width:.2f}h", va="center"
    )
